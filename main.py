...
def consume_item(item, t_inventory):
    if check_item(item, t_inventory):
        ...
        if t_inventory[item]<=0:
            print("현재 아이템의 수량이 적어서 사용 할 수 없습니다.")
        else:
            t_inventory[item] -= 1
            print(item+"의 수량은 "+str(t_inventory[item])+"입니다.")
    else:
...
def new_character(name, t_character):
    if check_charcater(name, t_character):
        print("이미 존재하는 캐릭터의 이름입니다.")
    else:
        inventory = {}
        t_character[name] = inventory

def check_charcater(name, t_character):
...
while True:
    ...
    if option == 0:
        ...
        temp_name = input("선택할 캐릭터의 이름을 입력해 주세요.")
        if check_charcater(temp_name, character):
            select_character = temp_name
            print(select_character + "가 선택되었습니다")
        else:
